...
/**
 * In-memory account type for audits (not persisted to DB)
 * Matches the shape expected by the rules engine
 */
...
export async function runAudit(
    ...
    try {
        ...
        // 2. Run rules engine
        const evaluatedAccounts = evaluateAccounts(accounts);
        ...
        // 7. Log lead to audit_leads table (using raw query as table may not be in types yet)
        // Note: Run scripts/create-audit-leads-table.sql in Supabase first
        try {
            await supabase.rpc('insert_audit_lead' as never, {
                p_email: email,
                p_company: company || null,
                p_mrr_at_risk: totalMrrAtRisk,
                p_at_risk_count: atRiskCount,
            } as never);
        } catch (insertErr) {
            // Non-blocking: log error but don't fail the audit
            console.warn('Could not log audit lead (table may not exist yet):', insertErr);
        }
